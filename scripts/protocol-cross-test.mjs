...
function cases() {
  ...
  return [
    ...
    ...[false, true].flatMap((stream) => [
      ...
      { name: `anthropic_thinking_${stream ? "stream" : "json"}`, protocol: "anthropic", stream, body: (id) => ({ model: requestedModel, stream, max_tokens: 1200, thinking: { type: "enabled", budget_tokens: 1024 }, messages: [{ role: "user", content: `Think briefly, then reply ${id} OK` }] }), expect: stream ? ["thinking_delta", "signature_delta"] : ["thinking", "signature"] },
    ]),
  ];
}
