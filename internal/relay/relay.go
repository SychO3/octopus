...
func Handler(inboundType inbound.InboundType, c *gin.Context) {
	...
	if isPassthroughStatus(lastResult.StatusCode) {
		...
		flushRelayFailure(c, hb, inAdapter, isStream, lastResult.StatusCode, "channel failed")
		return
	}
	...
	flushRelayFailure(c, hb, inAdapter, isStream, http.StatusBadGateway, "channel failed")
}

func flushRelayFailure(c *gin.Context, hb *earlyHeartbeat, inAdapter model.Inbound, isStream bool, statusCode int, message string) {
	if isStream {
		hb.FlushSSEOrProtocolError(c, inAdapter, statusCode, message)
		return
	}
